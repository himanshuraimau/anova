...
export const Page14Figure71 = () => {
  ...
  const generateCurve = (mean: number, stdDev: number) => {
    ...
    for (let x = 20; x <= 80; x += 0.5) {
      ...
      data.push({ x: parseFloat(x.toFixed(1)), y: parseFloat(y.toFixed(4)), mean1: null, mean2: null, mean3: null })
    }
    return data
  }

  const data = generateCurve(mu2, sigma)
  ...
  const minDiff = Math.min(Math.abs(mu1 - mu2), Math.abs(mu2 - mu3))
  const overlapPercent = Math.max(0, 100 - (maxDiff / (10 * sigma)) * 100)

  return (
    <div className="min-h-screen bg-cream px-4 py-12">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-5xl font-serif font-bold text-burgundy mb-2">Figure 7.1: Significantly Different</h1>
        <p className="text-muted-foreground font-serif mb-8">Well-separated group distributions</p>
        ...
        <Card className="border-2 border-gold p-6 bg-background">
          ...
          <p className={`font-serif text-foreground mb-3 ${textSizeClass}`}>
            When group distributions are <strong>well-separated</strong> (large differences relative to within-group variation), ANOVA will detect significant differences between groups.
          </p>
          <p className={`font-serif text-foreground ${textSizeClass}`}>
            In this scenario, you would <strong>reject the null hypothesis</strong> and conclude that group means are significantly different.
          </p>
        </Card>
      </div>
    </div>
